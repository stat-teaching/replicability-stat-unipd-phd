...
subtitle: "4. Inferential specification curve against clinically relevant effect"
...
Based on reviewer 2 comment, we include a specification curve analyses to see if the effect is distinctly different than a clinically relevant effect size of Hedges g = 0.24
...
specifications_inferential_spec_curve <- read.csv2(
  file = "data/tidy/specifications_cleaned.csv"
  ) %>% 
  filter(
    dependency == "aggregate" & (
      ma_method == "fe"  | ma_method == "reml" | ma_method == "pet-peese"
      )
    ) 
```

# Difference descriptive and inferential curve

```{r}
#sets <- as.vector(unique(specifications_inferential_spec_curve$set))
#sets <- lapply(strsplit(sets, ","), as.numeric)
#
#data_set_1 <- dat_boot[sets[[1]], ] 
#
#dat_descr <- data_set_1%>% 
...
#)
#
#mod_descr <- rma(yi = dat$yi, vi = dat$vi, method = "REML") 
#
#mod_inf <- rma(
#  yi = data_set_1$yi, # REML
#  vi = data_set_1$vi, 
#  method = "REML") #$b[[1]],
```
...
```{r}
set.seed(1234)
tictoc::tic(paste(iter, "iterations"))
...
```{r}
#save(res, file = "data/tidy/res_cre.RData")
#save(boot_tau, file = "data/tidy/boot_tau_cre.RData")
load("data/tidy/boot_tau_cre.RData")
```


```{r}
# List of percentage of estimated effects outside of respective 
# parametric bootstrap confidence ribbon
boot_outside <- # descriptive spec curve outside
c(
...
boot_outside <- # descriptive spec curve above grey area
c(
  tau_1 = mean(boot_tau$obs > boot_tau$upper1),
  tau_2 = mean(boot_tau$obs > boot_tau$upper2),
  tau_3 = mean(boot_tau$obs > boot_tau$upper3)
)
```
